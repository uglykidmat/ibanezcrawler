...
class tFPDF
{
    ...
    public function SetFillColor(int|string $r, ?int $g = null, ?int $b = null)
    {
        // Set color for all filling operations
        // case Hex value
        if ('string' == gettype($r)) {
            // list($r, $g, $b) = sscanf($r, "#%02x%02x%02x");
            $b = hexdec(substr($r, -2));
            $g = hexdec(substr($r, -4, 2));
            $r = hexdec(substr($r, -6, 2));
        }
    ...
    public function Cell(float $w, float $h = 0, string $txt = '', mixed $border = 0, int $ln = 0, string $align = '', bool $fill = false, mixed $link = '')
    {
        // Output a cell
        $txt = (string) $txt;
        $k = $this->k;
    ...
    protected function _putTTfontwidths($font, $maxUni)
    {
        ...
        for ($cid = $startcid; $cid < $cwlen; ++$cid) {
            ...
            if (
                ...
                || ("\00" == $font['cw'][$cid * 2] && "\00" == $font['cw'][$cid * 2 + 1])
            ) {
                continue;
            }
